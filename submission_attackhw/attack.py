...
class Attack:
    ...
    def eval_population(self, population, target_label):
        ...
        return:
            ...
            output: output of the model
            scores: the "fitness" of the image, measured as logits of the target label
            best_index: index of the best image in the population
        """
        output, scores = self.fitness(population, target_label)
        # --------------TODO--------------
        score_ranks = sorted(scores, reverse=True)  # Sort the scores from largest to smallest
        best_index = np.where(scores == score_ranks[0])[0][0]  # The index for the best scored candidate
        logits = [np.exp(score/self.temperature) for score in scores]  # Exponentiate the scores after incorporating temperature
        logit_sum = sum(logits)
        select_probs = [l/logit_sum for l in logits]  # Normalize the logits between 0-1
        # ------------END TODO-------------
        ...
        elite = [[population[best_index][0]]]

        # Survived: rest of the top genes that survived, mutated with some probability
        survived = []  # Survived, and mutate some of them
        s = 0
        while (len(elite) +  len(survived)) < (self.n_population * self.child_rate):
            i = np.where(scores == score_ranks[s])[0][0]
            # print(i, best_index)
            if i != best_index:
                mutate = np.random.choice([True, False], p=[self.mutate_rate, 1 - self.mutate_rate])
                if mutate:
                    survived.append(self.perturb(population[i]))
                else:
        ...
        children = []
        while (len(elite) + len(survived) + len(children)) < self.n_population:
            parents = np.random.choice(self.n_population, size=2, p=select_probs)
            children.append(self.crossover(population[parents[0]], population[parents[1]]))
        
        # print(np.shape(elite), np.shape(survived), np.shape(children))
